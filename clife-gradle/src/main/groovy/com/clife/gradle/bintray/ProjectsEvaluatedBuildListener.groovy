...
class ProjectsEvaluatedBuildListener extends BuildAdapter implements ProjectEvaluationListener {
    ...
    ProjectsEvaluatedBuildListener(BintrayUploadTask bintrayUpload, BintrayExtension bintrayExtension) {
        this.bintrayUpload = bintrayUpload
        this.bintrayUpload.extension = bintrayExtension
        this.bintrayUpload.extension.with {
            apiUrl = BintrayUploadTask.API_URL_DEFAULT
        }
    }
    ...
    void afterEvaluate(Project proj, ProjectState state) {
        ...
        if (bintrayPublish == null) {
            IllegalStateException e=  new IllegalStateException(String.format("Could not find %s in the root project", BintrayPublishTask.TASK_NAME))
            e.printStackTrace()
            throw e
        }
