...
class GUtil {
    public static String getVersion(){
        return new GUtil().pluginVersion
    }
    public static void addHeaders(Map<?,?> headers) {
        headers.put("User-Agent","gradle-bintray-plugin/${new GUtil().pluginVersion}")
    }

    public String getPluginVersion() {
        Properties props = new Properties()
        final String fileName = "clife.plugin.release.properties"
        props.load(getClass().classLoader.getResource(fileName).openStream())
        props.get('version')
    }
